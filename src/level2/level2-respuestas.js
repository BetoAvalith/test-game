export function obtenerMayor(x, y) {
  ...
  export function conection(status) {
    ...
    return userStatus[status] ? userStatus[status] : userStatus[3];
  }
  ...
  export function saludo(idioma) {
    ...
    return saludos[idioma] ? saludos[idioma] : 'Hola!';
  }
  ...
  export function tablaDelSeis(){
    ...
    //Escribe tu código aquí
    const tablaArray = new Array(11).fill(0)   
    return tablaArray.map((numero, index) => 6 * index)
  }
